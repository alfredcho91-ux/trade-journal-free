...
DAILY_TABLE_NAME = "daily_journal_entries"
...
OPTIONAL_SCHEMA_COLUMNS = {
    ...
    "emotion_during": "TEXT",
    "emotion_after": "TEXT",
    "confidence_score": "INTEGER CHECK (confidence_score BETWEEN 1 AND 5 OR confidence_score IS NULL)",
    "focus_score": "INTEGER CHECK (focus_score BETWEEN 1 AND 5 OR focus_score IS NULL)",
    "fomo": "INTEGER CHECK (fomo IN (0, 1) OR fomo IS NULL)",
    "revenge_trade": "INTEGER CHECK (revenge_trade IN (0, 1) OR revenge_trade IS NULL)",
...
# These values are authored after import and must never be reconstructed from
# exchange history. Legacy ``tags`` are intentionally excluded: current
# importers use them as source/type classifiers, while ``mistakes`` is a user
# annotation and is never generated by sync.
USER_OWNED_JOURNAL_COLUMNS = frozenset({
    "emotion",
    "notes",
    "mistakes",
    "planned_stop_pct",
...
if USER_OWNED_JOURNAL_COLUMNS.intersection(EXCHANGE_REFRESH_COLUMNS):
    raise RuntimeError("Journal-owned fields cannot be refreshed by exchange sync")
    ...
    with SCHEMA_LOCK:
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS {DAILY_TABLE_NAME} (
                ...
                market_bias TEXT,
                session_plan TEXT,
                max_daily_loss REAL CHECK (max_daily_loss > 0 OR max_daily_loss IS NULL),
                max_trade_count INTEGER CHECK (max_trade_count > 0 OR max_trade_count IS NULL),
...
def _normalize_column_value(column: str, value: Any) -> Any:
    ...
    if column in {"fomo", "revenge_trade"}:
        value = _clean_value(value)
        if value is None:
            return None
        ...
        if type(value) is int and value in (0, 1):
            return value
        raise ValueError(f"{column} must be null, false, or true")
