...
- OHLCV의 canonical `volume`은 Binance USDT-M Futures의 기초자산 수량입니다. 원본의 `quote_volume`, `taker_buy_base_volume`, `taker_buy_quote_volume`도 정규화·캐시에 보존하며, canonical volume이 누락된 원본 봉은 `0`으로 만들지 않고 사용할 수 없는 봉으로 제외합니다.
- 진입 시점 feature는 진입 전에 종료된 봉만 사용합니다. 진행 중인 진입 봉의 최종 거래량은 진입 당시 알 수 있었던 값으로 사용하지 않습니다. 종목 간 raw 거래량 절대값은 직접 비교하지 않으며, 향후 거래량 분석은 종목 내부 상대값·백분위·정규화 값을 우선합니다.
- Entry RVOL20은 진입 직전 완료봉 거래량을 그 이전 20개 완료봉 평균으로 나눈 종목 내부 상대 거래량입니다. 진행 중인 진입 봉의 최종 거래량은 사용하지 않으며, 거래량 누락·20봉 부족·0 평균은 `—`로 표시합니다. RVOL20은 현재 진입 당시 시장 맥락 표시용이며 Quality, Attribution, Optimizer 또는 공식 매매 판단 입력에는 연결하지 않습니다.
