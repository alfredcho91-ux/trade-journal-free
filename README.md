...
- 과거 종료 거래의 당시 계획을 순서대로 복원하고 실제 실행과 비교하는 Plan Lab. 거래 전 서버에 저장한 계획은 별도 검증 표식으로 구분
...
- 회고 입력 Plan은 당시 기억을 구조화하는 사후 분석 자료입니다. 실제 Entry 가격에 계획 SL/TP를 적용하고 실제 종료 뒤 최대 40시간까지 경로를 재생하며, 결과를 미래 추천 신호로 해석하지 않습니다.
