...
- `frontend/src/pages/JournalPage.tsx`: Deepcoin 동기화와 종료 거래 목록
...
- `backend/modules/deepcoin/`: 서명된 읽기 전용 API 호출과 체결 저장
...
`backend/main.py`는 위 세 라우터만 등록합니다. AI, 백테스트, 스캐너, 전략 분석 라우터는 무료판에 포함하지 않습니다.

## 보안 경계

- Deepcoin 자격 증명은 서버의 환경 변수로만 읽습니다.
- API 응답은 키·secret·passphrase를 반환하지 않습니다.
- Deepcoin 동기화는 읽기 전용 endpoint만 사용합니다.
