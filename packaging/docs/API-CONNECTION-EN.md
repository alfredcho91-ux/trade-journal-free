...
Trade Journal supports Deepcoin, Binance, Bybit, and OKX. Some exchanges also require a Passphrase.
...
5. Enter a Passphrase when required by the exchange, such as Deepcoin or OKX. A Passphrase is the separate password you created when making the API key.
...
- Select `SWAP` for derivatives history or `SPOT` for spot history.
