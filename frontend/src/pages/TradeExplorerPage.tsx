...
import { evidenceMinimumReturnLabel, matchesEvidenceDirection, useEvidenceNavigation, type EvidenceRequest } from '../features/tradeAnalysis/evidenceNavigation';
...
type Direction = EvidenceRequest['direction'];
...
export default function TradeExplorerPage() {
  ...
  const [minimumAbsNetReturnPct, setMinimumAbsNetReturnPct] = useState(() => evidenceRequest?.minimumAbsNetReturnPct || 0);
  ...
  useEffect(() => {
    ...
    if (minimumAbsNetReturnPct !== evidenceRequest.minimumAbsNetReturnPct) {
      setMinimumAbsNetReturnPct(evidenceRequest.minimumAbsNetReturnPct);
    }
  }, [direction, evidenceRequest, minimumAbsNetReturnPct, period]);
  ...
  const qualityQuery = useQuery({
    ...
    queryFn: () => getJournalQualityAnalysis({ start_time: startTime as number, end_time: endTime as number, min_abs_net_return_pct: minimumAbsNetReturnPct }),
  ...
  const periodTrades = useMemo(() => {
    ...
    return buildAnalyzedTrades(entries, excursions)
      ...
      .filter((trade) => isIncludedByMinimumAbsNetReturn(trade.entry, minimumAbsNetReturnPct));
  }, [entries, minimumAbsNetReturnPct, period, qualityQuery.data?.items]);
  const directionTrades = useMemo(
    () => periodTrades.filter((trade) => matchesEvidenceDirection(direction, trade.entry.direction)),
    [direction, periodTrades],
  );
  const defaultRequest = useMemo<EvidenceRequest>(() => ({
    title: isKo ? '현재 필터의 종료 거래' : 'Closed trades in the current filter',
    filterLabel: `${direction.toUpperCase()} · ${period.start} ~ ${period.end} · ${evidenceMinimumReturnLabel(minimumAbsNetReturnPct, isKo)} · ${directionTrades.length}${isKo ? '건' : ' trades'}`,
    tradeIds: directionTrades.flatMap((trade) => trade.entry.id == null ? [] : [trade.entry.id]),
    period,
    direction,
    minimumAbsNetReturnPct,
  }), [direction, directionTrades, isKo, minimumAbsNetReturnPct, period]);
  ...
  const openEvidence = (title: string, tradeIds: number[]) => {
    setEvidenceRequest({
      title,
      filterLabel: `${direction.toUpperCase()} · ${period.start} ~ ${period.end} · ${evidenceMinimumReturnLabel(minimumAbsNetReturnPct, isKo)} · ${tradeIds.length}${isKo ? '건' : ' trades'}`,
    ...
    <div className="space-y-5">
      <header className="flex flex-wrap items-end justify-between gap-3 border-b border-dark-700 pb-4">
        ...
        <div className="flex border border-dark-700 bg-dark-950/50 p-1">
          {(['All', 'Long', 'Short'] as Direction[]).map((item) => <button key={item} type="button" onClick={() => { setDirection(item); clearEvidenceRequest(); }} className={`min-h-8 px-3 text-xs font-medium ${direction === item ? 'bg-primary-500/20 text-primary-100' : 'text-dark-400 hover:text-white'}`}>{item.toUpperCase()}</button>)}
        </div>
      </header>
      <div className="text-xs text-dark-500">{direction.toUpperCase()} · {period.start} ~ {period.end} · {evidenceMinimumReturnLabel(minimumAbsNetReturnPct, isKo)} · {isKo ? '원본 분석 범위 유지' : 'Source analysis scope preserved'}</div>
      ...
      <AnalysisAccordion title={isKo ? '거래별 청산 복기 보기' : 'View exit review by trade'}>
        <TradeExitReviewList entries={entries} qualityItems={qualityQuery.data?.items || []} direction={direction === 'All' ? undefined : direction} isKo={isKo} onViewAll={(journalIds) => openEvidence(isKo ? '거래별 청산 복기 근거 거래' : 'Trade exit review evidence', journalIds)} />
      </AnalysisAccordion>
    </div>
  );
}
