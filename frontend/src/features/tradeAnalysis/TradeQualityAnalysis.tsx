  ...
  return (
    <div className="space-y-4">
      <section className="border border-dark-700 bg-dark-900/20 p-4">
        ...
        <div className="mt-4 grid gap-px bg-dark-700 md:grid-cols-2 xl:grid-cols-4">
          <div className="bg-dark-950 p-3">
            <div className="flex items-center gap-1.5 text-[11px] text-bull"><TrendingUp className="h-3.5 w-3.5" />{isKo ? '평균 손익 상위 Regime' : 'Highest Average PnL Regime'}</div>
          ...
          <div className="bg-dark-950 p-3">
            <div className="flex items-center gap-1.5 text-[11px] text-bear"><TrendingDown className="h-3.5 w-3.5" />{isKo ? '평균 손익 하위 Regime' : 'Lowest Average PnL Regime'}</div>
          ...
          <div className="bg-dark-950 p-3">
            <div className="text-[11px] text-dark-500">{isKo ? '청산 성향' : 'Exit Tendency'}</div>
            ...
            <div className="mt-1 text-xs text-dark-400">Capture {number(summary.average_capture_ratio_pct)}%</div>
          </div>
          <div className="bg-dark-950 p-3">
            <div className="text-[11px] text-dark-500">{isKo ? '개선 우선순위' : 'Primary Issue'}</div>
        ...
        <div className="mt-4 grid gap-3 text-xs lg:grid-cols-2">
          <div className="border-t border-dark-700 pt-3">
            <div className="mb-2 font-medium text-dark-200">{isKo ? '추세 정렬 vs 충돌' : 'Aligned vs Conflicting Trends'}</div>
            <div className="flex justify-between text-dark-400"><span>{isKo ? '정렬' : 'Aligned'} <span className="text-dark-600">n={aligned?.trade_count || 0}</span></span><span className="font-mono">{number(aligned?.win_rate_pct)}% · {signed(aligned?.average_pnl)} USDT</span></div>
            <div className="mt-1 flex justify-between text-dark-400"><span>{isKo ? '충돌' : 'Conflict'} <span className="text-dark-600">n={conflict?.trade_count || 0}</span></span><span className="font-mono">{number(conflict?.win_rate_pct)}% · {signed(conflict?.average_pnl)} USDT</span></div>
          </div>
          <div className="border-t border-dark-700 pt-3">
            <div className="mb-2 font-medium text-dark-200">{isKo ? '추세 순방향 vs 역방향' : 'With-Trend vs Counter-Trend'}</div>
            <div className="flex justify-between text-dark-400"><span>{isKo ? '순방향' : 'With trend'} <span className="text-dark-600">n={withTrend?.trade_count || 0}</span></span><span className="font-mono">{number(withTrend?.win_rate_pct)}% · {signed(withTrend?.average_pnl)} USDT</span></div>
            <div className="mt-1 flex justify-between text-dark-400"><span>{isKo ? '역방향' : 'Counter trend'} <span className="text-dark-600">n={counterTrend?.trade_count || 0}</span></span><span className="font-mono">{number(counterTrend?.win_rate_pct)}% · {signed(counterTrend?.average_pnl)} USDT</span></div>
          </div>
        </div>
      </section>
      ...
      <details className="group">
        <summary className="flex cursor-pointer list-none items-center justify-between border border-dark-700 bg-dark-900/30 px-4 py-3 text-sm font-medium text-dark-200 hover:text-white">
          <span>{isKo ? '추세·청산 상세 분석' : 'Regime and Exit Details'}</span>
          <span className="flex items-center gap-2 text-[11px] font-normal text-dark-500">{analysis.regimes.length}{isKo ? '개 Regime' : ' regimes'}<ChevronDown className="h-4 w-4 transition-transform group-open:rotate-180" /></span>
        </summary>
        <div className="mt-3 space-y-3">
          <section className="border border-dark-700 bg-dark-900/20 p-4">
            <h2 className="text-sm font-semibold text-white">{isKo ? 'Weekly / Daily / 4H Regime' : 'Weekly / Daily / 4H Regimes'}</h2>
          ...
          <table className="w-full min-w-[920px] text-xs">
            <thead className="text-dark-500"><tr className="border-b border-dark-700"><th className="py-2 text-left">Regime</th><th className="py-2 text-right">{isKo ? '거래' : 'Trades'}</th><th className="py-2 text-right">{isKo ? '승률' : 'Win rate'}</th><th className="py-2 text-right">PF</th><th className="py-2 text-right">MFE / MAE</th><th className="py-2 text-right">{isKo ? '조기청산' : 'Early exits'}</th><th className="py-2 text-right">Capture</th><th className="py-2 text-right">{isKo ? '우위 청산' : 'Best exit'}</th></tr></thead>
          ...
          <section className="border border-dark-700 bg-dark-900/20 p-4">
            <h2 className="text-sm font-semibold text-white">{isKo ? '청산 방식 비교' : 'Exit Method Comparison'}</h2>
          ...
          <div>
            <h3 className="text-xs font-medium text-dark-200">{isKo ? '실제 청산 대비 추가 홀딩' : 'Actual Exit vs Additional Holding'}</h3>
          ...
          <div>
            <h3 className="text-xs font-medium text-dark-200">{isKo ? '보조지표 가상 청산' : 'Indicator-Based Virtual Exits'}</h3>
