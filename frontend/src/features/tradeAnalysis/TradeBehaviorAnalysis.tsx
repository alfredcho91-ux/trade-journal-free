...
function planStatus(item: JournalBehaviorAnalysisData['items'][number], isKo: boolean): string {
  if (!item.plan.eligible_for_exit_plan_review) return isKo ? '현재 기록: 과거 진입 전 상태 검증 불가' : 'Current note: historical pre-trade status unavailable';
...
function StatsLine({ stats, isKo }: { stats: JournalBehaviorStats; isKo: boolean }) {
  return (
    <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs sm:grid-cols-4">
      <div className="col-span-full mb-1"><SampleBadge count={stats.pnl_sample_count || stats.trade_count} isKo={isKo} /></div>
      <span className="text-dark-400">{isKo ? '거래' : 'Trades'} <b className="ml-1 font-mono text-dark-100">{stats.trade_count}</b></span>
    ...
    <section className="border border-dark-700 bg-dark-900/30 p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-[11px] text-dark-500">{isKo ? '실현 손실은 여러 원인에 나눠 반영합니다. 조기·늦은 청산의 놓친 움직임은 금액이 아닌 가격 변동률로 따로 보여줍니다. 이 요약은 선택 기간의 LONG·SHORT 전체 기준입니다.' : 'Realized losses are shared across causes. Exit opportunity is shown separately as price movement, not as realized PnL. This summary covers both LONG and SHORT trades in the selected period.'}</p>
        </div>
...
function RuleEditor({ data, isKo }: { data: JournalBehaviorAnalysisData; isKo: boolean }) {
  ...
  return <section className="border border-dark-700 bg-dark-900/20 p-4"><div><h2 className="text-sm font-semibold text-white">{isKo ? '규칙 준수 분석' : 'Rule Compliance'}</h2><p className="mt-1 text-[11px] text-dark-500">{isKo ? '완료된 진입 시점 추세를 사용합니다. 수정 가능한 과거 계획의 진입 전 상태는 검증할 수 없어 계획 규칙은 판정 불가입니다.' : 'Checks use completed entry-time trends. Mutable legacy plans cannot prove their historical pre-trade values, so plan rules stay unknown.'}</p></div>
