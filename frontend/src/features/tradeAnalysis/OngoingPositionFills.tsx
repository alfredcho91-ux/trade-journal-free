  ...
  return <section className="border border-amber-400/30 bg-amber-400/5 p-4">
    ...
    {positions.length > 0 && <div className="mt-3 overflow-x-auto">
      <table className="w-full min-w-[680px] text-xs">
        ...
        <tbody>{positions.map((position) => <tr key={`${position.exchange}-${position.position_id || `${position.symbol}-${position.direction}`}`} className="border-b border-dark-800 last:border-0"><td className="py-2 text-dark-300">{position.exchange}</td><td className="py-2 text-dark-300">{dateLabel(position.opened_at)}</td><td className="py-2 text-dark-200">{position.symbol}</td><td className={`py-2 text-center ${position.direction === 'Long' ? 'text-bull' : 'text-bear'}`}>{position.direction}</td><td className="py-2 text-right font-mono">{position.average_price?.toLocaleString(undefined, { maximumFractionDigits: 2 }) || '-'}</td><td className="py-2 text-right font-mono">{position.size.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td><td className="py-2 text-right font-mono">{position.leverage ? `${position.leverage}x` : '-'}</td><td className={`py-2 text-right font-mono ${(position.unrealized_pnl || 0) >= 0 ? 'text-bull' : 'text-bear'}`}>{position.unrealized_pnl == null ? '-' : `${position.unrealized_pnl >= 0 ? '+' : ''}${position.unrealized_pnl.toLocaleString(undefined, { maximumFractionDigits: 2 })} USDT`}</td><td className="py-2 text-center"><span className="border border-amber-400/30 bg-amber-400/10 px-1.5 py-0.5 text-[10px] text-amber-300">{isKo ? '진행중' : 'OPEN'}</span></td></tr>)}</tbody>
      </table>
    </div>}
    {fills.length > 0 && <div className="mt-4 overflow-x-auto">
      ...
      <table className="w-full min-w-[680px] text-xs"><thead className="text-dark-500"><tr className="border-b border-dark-700"><th className="py-2 text-left">{isKo ? '체결시각' : 'Fill time'}</th><th className="py-2 text-left">{isKo ? '종목' : 'Symbol'}</th><th className="py-2 text-center">{isKo ? '방향' : 'Side'}</th><th className="py-2 text-right">{isKo ? '체결가' : 'Price'}</th><th className="py-2 text-right">{isKo ? '수량' : 'Size'}</th><th className="py-2 text-center"><span className="sr-only">{isKo ? '리포트' : 'Report'}</span></th></tr></thead><tbody>{fills.map((row) => <tr key={row.id || row.external_id || `${row.symbol}-${row.datetime}`} className="border-b border-dark-800 last:border-0"><td className="py-2 text-dark-300">{dateLabel(row.datetime)}</td><td className="py-2 text-dark-200">{row.symbol || '-'}</td><td className={`py-2 text-center ${row.direction === 'Long' ? 'text-bull' : 'text-bear'}`}>{row.direction || '-'}</td><td className="py-2 text-right font-mono">{row.entry_price?.toLocaleString(undefined, { maximumFractionDigits: 2 }) || '-'}</td><td className="py-2 text-right font-mono">{row.size?.toLocaleString(undefined, { maximumFractionDigits: 2 }) || '-'}</td><td className="py-2 text-center"><button type="button" onClick={() => setSelected(row)} className="text-dark-400 hover:text-white" title={isKo ? '거래 리포트 열기' : 'Open trade report'}><CandlestickChart className="mx-auto h-4 w-4" /></button></td></tr>)}</tbody></table>
    </div>}
