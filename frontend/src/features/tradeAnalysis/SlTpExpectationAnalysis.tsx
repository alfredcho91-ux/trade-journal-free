      ...
      <div className="border-t border-dark-700">
        ...
        {query.isLoading ? (
          ...
          <div>
            ...
            <details className="px-4 py-3 text-[10px] leading-4 text-dark-600" open><summary className="flex cursor-pointer items-center gap-1 text-dark-500"><Info className="h-3 w-3" />{isKo ? '계산 기준과 데이터 범위' : 'Method and coverage'}</summary><p className="mt-2">{isKo ? '각 거래의 최초 진입부터 실제 종료까지 완전히 포함된 해당 거래소 5분봉을 우선 시간순으로 재생하고, 조회할 수 없을 때만 Binance Spot 대체 데이터를 사용합니다. 같은 5분봉에서 SL과 TP가 모두 닿으면 순서를 알 수 없으므로 ambiguous로 기록하고 SL 처리합니다. 결과는 방향성 가격수익률에서 해당 거래의 실제 수수료율을 차감하며, 가상 종료 시점의 펀딩과 슬리피지는 포함하지 않습니다. 누적 수익과 Max Drawdown은 매 거래에 동일 자본을 순차 재투자한 복리 가정입니다.' : 'The simulator first replays fully-contained 5m candles from the trade exchange and uses labeled Binance Spot fallback data only when necessary. Same-bar collisions are marked ambiguous and conservatively stopped. Recorded fee rate is used as a proxy; funding and slippage are excluded.'}</p><p className="mt-1">{isKo ? `경로 ${query.data?.coverage.analyzed_positions || 0}/${query.data?.coverage.closed_positions_considered || 0}건 · 수수료 반영 ${query.data?.coverage.fee_proxy_positions || 0}건 · 과거 70% 선택 / 최근 30% 검증` : `${query.data?.coverage.analyzed_positions || 0}/${query.data?.coverage.closed_positions_considered || 0} paths · fees ${query.data?.coverage.fee_proxy_positions || 0} · 70/30 train-validation`}</p>{(query.data?.warnings.length || 0) > 0 && <div className="mt-1 space-y-0.5 text-amber-400/80">{query.data?.warnings.map((warning) => <div key={warning}>{warning}</div>)}</div>}</details>
          </div>
        )}
      </div>
    </details>
