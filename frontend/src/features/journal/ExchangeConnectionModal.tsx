    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/75 p-4" role="dialog" aria-modal="true" aria-label={isKo ? `${exchange.name} API 연결` : `${exchange.name} API connection`}>
      <form className="w-full max-w-md border border-dark-600 bg-dark-950 shadow-2xl" onSubmit={(event) => { event.preventDefault(); if (canSave) onSave({ api_key: apiKey, secret_key: secretKey, passphrase }); }}>
        ...
        <div className="space-y-4 px-5 py-4">
          <label className="block text-xs text-dark-300">{isKo ? 'API 키' : 'API Key'}<input autoComplete="off" value={apiKey} onChange={(event) => setApiKey(event.target.value)} className="mt-1.5 w-full border border-dark-700 bg-dark-900 px-3 py-2 text-sm text-white" /></label>
          <label className="block text-xs text-dark-300">{isKo ? '시크릿 키' : 'Secret Key'}<input type="password" autoComplete="new-password" value={secretKey} onChange={(event) => setSecretKey(event.target.value)} className="mt-1.5 w-full border border-dark-700 bg-dark-900 px-3 py-2 text-sm text-white" /></label>
          {exchange.requires_passphrase && <label className="block text-xs text-dark-300">{isKo ? '패스프레이즈' : 'Passphrase'}<input type="password" autoComplete="new-password" value={passphrase} onChange={(event) => setPassphrase(event.target.value)} className="mt-1.5 w-full border border-dark-700 bg-dark-900 px-3 py-2 text-sm text-white" /></label>}
